...
# 1. إعدادات الواجهة (الهيبة العمانية)
...
# 3. اختيار الموديل (استخدام مسمى قديم ومستقر جداً يهرب من فخ 404)
try:
    # جربنا نستخدم gemini-pro مباشرة بدون أي إضافات
    model = genai.GenerativeModel('gemini-pro')
except:
    # حل أخير لو السيرفر لازال معاند
...
if prompt := st.chat_input("سولف مع AHMED AI PRO..."):
    ...
    try:
        with st.spinner("أحمد AI يفكّر بعبقرية الصالحي..."):
            # تعليمات الأسلوب الأسطوري (Few-Shot)
            instruction = (
                "أنت 'أحمد AI'. مبرمجك هو العبقري أحمد بن بدر الصالحي. "
                "تكلم بلهجة عمانية راقية، ذكية، وفخورة. "
                "قلد هذا الأسلوب بالضبط: "
                "1. عند الترحيب: 'يا هلا والله ومسهلا! حياك الله يا راعي الواجب'. "
                "2. عند ذكر المبرمج: 'بفضل البرمجة العبقرية اللي وضعها فيني الأستاذ أحمد الصالحي'. "
                "3. إذا كان اسم المستخدم أحمد: 'حيّاك الله يا سميّي!'. "
                "4. الخاتمة دائماً حماسية: 'بإذن الله بنكسر الدنيا!'. "
                "ممنوع الفصحى، كن ذكياً كأنك بوبدر نفسه."
            )
        ...
        # لو طلع خطأ، بنخلي الرسالة تشرح للمستخدم إن بوبدر جالس يطور النظام
        st.warning("علوه يا جماعة، بوبدر الحين جالس يسوي صيانة وتحديثات عالمية للنظام.. جربوا بعد شوي! 🛠️")
        st.error(f"تفاصيل للمبرمج بوبدر: {e}")
